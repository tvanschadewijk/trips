  ...
  if (loading) {
    return (
      <div className="dash" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100dvh' }}>
        <span style={{ color: '#8a8f98', fontSize: 14 }}>Loading...</span>
      </div>
    );
  }
    ...
    <div className="dash">
      <nav className="dash-nav">
        <div className="dash-nav-inner">
          <Link href="/" className="dash-logo">Our Trips<span className="logo-to">.To</span> <span className="logo-suffix">{trips.length > 0 ? `${trips[0].name}${trips.length > 1 ? ` and ${trips.length - 1} more` : ''}` : '?'}</span></Link>

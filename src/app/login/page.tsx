...
export default function LoginPage() {
  ...
  function handleLocalPreview() {
    const trips = getLocalPreviewTrips();
    sessionStorage.setItem('dash-email', 'local-preview@example.com');
    sessionStorage.setItem('dash-trips', JSON.stringify(trips));
    router.push('/dashboard');
  }

  if (!ready) return null;
  const localPreview = isLocalPreviewWithoutSupabase();
        ...
        <div className="login-card">
          ...
          <p className="login-desc">
            {isSupabaseConfigured()
              ...
              : 'Supabase is not configured, so use the local preview trip.'}
          </p>

          {!isSupabaseConfigured() && (
            <div className="login-config-warning">
              Local preview uses sample data in this browser. Add <code>NEXT_PUBLIC_SUPABASE_URL</code> and <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> to <code>.env.local</code> to enable Google sign-in.
            </div>
          )}

          {localPreview ? (
            <button type="button" className="login-btn-google login-btn-local" onClick={handleLocalPreview}>
              Open local preview
            </button>
          ) : (

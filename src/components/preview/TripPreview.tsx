...
export default function TripPreview({ trips: initialTrips, onDelete, autoOpen, shareId }: TripPreviewProps) {
  ...
  // Share trip via native share API or show share sheet
  const shareUrl = shareId ? `${typeof window !== 'undefined' ? window.location.origin : ''}/t/${shareId}` : '';
  const shareText = trip ? `Check out this trip: ${trip.name} — ${trip.subtitle}` : '';

  function handleShare() {
    if (navigator.share) {
      navigator.share({ title: trip?.name || 'Trip', text: shareText, url: shareUrl }).catch(() => {});
    } else {
      setShareSheetOpen(true);
    }
  }

  function shareVia(channel: 'whatsapp' | 'email' | 'copy') {
    const text = encodeURIComponent(shareText);
    const url = encodeURIComponent(shareUrl);
    if (channel === 'whatsapp') {
      window.open(`https://wa.me/?text=${text}%20${url}`, '_blank');
    } else if (channel === 'email') {
      window.location.href = `mailto:?subject=${encodeURIComponent(trip?.name || 'Trip')}&body=${text}%0A${url}`;
    } else {
  ...
  return (
    <div className="trip-app" ref={appRef}>
      ...
      {activeTripIndex !== null && (
        ...
        >
          ...
          <div className={`nav-bar ${isHero ? 'over-hero' : ''}`}>
            ...
            {shareId && (
              <button className="nav-share" onClick={handleShare} aria-label="Share trip">
      ...
      {/* Share sheet */}
      {shareSheetOpen && (
        <div className="share-overlay" role="dialog" aria-modal="true" aria-label="Share trip">
          <div className="share-backdrop" onClick={() => setShareSheetOpen(false)} />
          <div className="share-sheet">
            <div className="share-sheet-title">Share this trip</div>
            <div className="share-sheet-options">
              <button className="share-option" onClick={() => shareVia('whatsapp')}>
              ...
              <button className="share-option" onClick={() => shareVia('email')}>
                <div className="share-option-icon share-option-email">
              ...
              <button className="share-option" onClick={() => shareVia('copy')}>
            ...
            <button className="share-sheet-cancel" onClick={() => setShareSheetOpen(false)}>Cancel</button>
          </div>
        </div>
      )}

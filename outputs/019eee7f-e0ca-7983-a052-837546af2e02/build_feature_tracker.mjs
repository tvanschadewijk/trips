...
const stories = [
  ...
  ["US-087", "Maps", "Overview/day maps", "Clickable map markers", "As a traveler, I want map pins, route points, and itinerary place labels to be clickable so I can inspect the matching stop or place.", "Overview and day maps expose interactive markers or equivalent controls; clicking a marker or linked itinerary place focuses the correct map target, opens readable place/stop information, and view-all returns the map to all visible locations.", "src/components/preview/ItineraryMap.tsx; src/components/preview/TripPreview.tsx; src/lib/day-map.ts", "Mapped", "P1", "Browser/Maps", "Needs Retest", "", "Added after map audit prompt. Existing smoke verifies map rendering, but marker click/focus behavior needs a dedicated interaction test."],
  ["US-088", "Maps", "Geocoding/search", "Location accuracy and disambiguation", "As a traveler, I want every hotel, restaurant, activity, and route stop to appear in the correct city/country on the map.", "Stored lat/lng and route atlas coordinates are preferred; search queries include trip/day/city context; location bias/restriction prevents ambiguous names from resolving to unrelated cities; a place like Hotel Casablanca must not resolve to Amsterdam when the itinerary context is Casablanca.", "src/components/preview/ItineraryMap.tsx; src/lib/day-map.ts; src/lib/trip-route.ts; src/lib/day-map.test.ts; src/lib/trip-route.test.ts", "Mapped", "P1", "Browser/Data", "Needs Retest", "", "Existing unit tests cover partial disambiguation, fallback points, known aliases, and context-rich search targets; add explicit ambiguous-hotel fixtures and browser map validation."],
  ["US-089", "Maps", "Map details and external links", "Place metadata and map links", "As a traveler, I want map detail cards and external map links to describe the same real-world place shown in the itinerary.", "Map details preserve the intended title, type, day/time context, address when known, place_id/google_maps_url when supplied, and safe external map/search links; links must not use stale coordinates, stale labels, or a nearby-but-wrong result.", "src/components/preview/ItineraryMap.tsx; src/components/preview/TripPreview.tsx; src/lib/types.ts; src/lib/day-map.ts", "Mapped", "P1", "Browser/Maps", "Needs Retest", "", "Added as explicit map QA coverage for correct marker metadata and outbound map links."],
];
...
summary.getRange("D5:H9").values = [
  ...
  ["Test every user story", "Needs Map Retest", "Codex", "Browser/API/test commands", "Map-click, geocoding, and link-correctness stories added as Needs Retest."],
  ["Fix logistical/UX errors", "Completed", "Codex", "Code diffs and issue IDs", "ISS-001, ISS-002, and ISS-003 fixed and retested."],
  ["Retest every behavior", "Needs Map Retest", "Codex", "Retest Status and notes", "Prior post-fix verify, smoke, API, and interaction checks passed; new map-specific cases now queued."],
